...
contract GameLogic is IGameLogic {
    ...
    function _awardPots() private {
        ...
        for (uint8 i = 0; i < PokerConstants.MAX_PLAYERS; i++) {
            ...
            if (playerAddr != address(0)) {
                ...
                console.log(
                    'Player at position %s bet: %s status: %s',
                    i,
                    player.currentBet,
                    uint8(player.status)
                );
    ...
    function _determineWinnerForPot(
        uint256 potIndex
    ...
    function _processFold(address player) private {
        ...
        console.log(
            'Processing fold for player at position',
        ...
        for (uint8 i = 0; i < PokerConstants.MAX_PLAYERS; i++) {
            ...
            if (playerAddress != address(0)) {
                ...
                if (player.currentBet > 0) {
                    console.log(
                        'Resetting currentBet for player at position',
                        i
                    );
                    player.currentBet = 0;
