...
contract TournamentLogic is ITournamentLogic {
    ...
    ) external override onlyValidPlayers(players) {
    ...
    function calculateNextBlinds(
        ...
        (uint256 smallBlind, uint256 bigBlind, , , , , , , , , ) = stateStorage
            .getTournamentStateValues();
        uint256 currentLevel = getCurrentBlindLevel();
    ...
    function processElimination(address player) external override {
        ...
        (
            ...
            ,
            bool isPaused, // currentBlindLevel
        ...
        if (activeCount == 2) {
            // If activeCount will become 1 after elimination
            _completeTournament();
        }
    }
